...
def draw_time_hist(probe: ProbeBase, c: ConcatInfo, r, theta, fig, ax):
    ...
    # 随机采样1000个svf为True的点，固定随机种子
    svf_idx = np.where(svf)[0]
    np.random.seed(0)
    if len(svf_idx) > 1000:
        svf_idx = np.random.choice(svf_idx, 1000, replace=False)
    svrs = c.v_rs[svf_idx]
    svths = c.v_thetas[svf_idx]
    ts = np.linspace(0, 1000, num=1001)
    n_t = len(ts)
    ss = np.average(probe.get_lc(
                    np.repeat(svrs.reshape(1,len(svrs)), n_t, axis=0), 
                    np.repeat(svths.reshape(1,len(svths)), n_t, axis=0),
                    np.repeat(ts.reshape(n_t,1), len(svrs), axis=1)), 
                    axis=1)
    ...
    if n_v != 0:
        ...
        ax.hist(
            ...
            range=time_range,
            bins=bins,
            weights=np.repeat(1.0 / n_v / 10, len(sts)),
            label="histogram",
        )
...
if __name__ == "__main__":
    ...
    psr.add_argument("--model", type=str, help="model file")
    psr.add_argument("--type", type=str, help="model type", choices=["GAM", "GBM", "CNN"], default="GAM")
    psr.add_argument("-o", "--output", dest="opt", type=str, help="output file")
    args = psr.parse_args()

    if args.type == "GAM":
        from probe_gam import Probe
    elif args.type == "GBM":
        from probe_gbm import Probe
    elif args.type == "CNN":
        from probe_cnn import Probe
    ...
    if args.command == "draw":
        ...
        with PdfPages(args.opt) as pp:
            probe = get_probe(args.model)

            
            figures = []

            figures.append(draw_log_pie_fig(probe))
            figures.append(draw_real_pie_fig(concat))
            figures.append(draw_quotient_fig(probe, concat))
