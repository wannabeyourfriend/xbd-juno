...
class ProbeBase(metaclass=ABCMeta):
    ...
    def is_consistent(self, v_rs, v_thetas) -> bool:
        ...
        atol = 1e-8
        rtol = 5e-1
        diff = np.abs(marginal - integral)
        threshold = atol + rtol * np.abs(integral)
        failed_mask = diff > threshold
        print(f"失败点数量：{np.sum(failed_mask)}/{NV}")
        print(f"最大实际差异：{np.max(diff/threshold):.1%}")
        print(f"{marginal[0:9]}, {integral[0:9]}")
        print(np.allclose(marginal[0:9], integral[0:9], rtol=5e-1))
        # 计算所有点的相对差异
        rel_diff = np.abs(marginal - integral) / np.abs(integral)
        max_idx = np.argmax(rel_diff)
    
        # 输出最大差异点信息
        print(f"\n最大差异点："
        f"r={rs[max_idx]:.4f}, θ={thetas[max_idx]:.4f} | "
        f"get_mu={marginal[max_idx]:.4e}, ∫get_lc={integral[max_idx]:.4e} | "
        f"相对差异={rel_diff[max_idx]:.2%}")
        return np.allclose(marginal, integral, rtol=5e-1)
